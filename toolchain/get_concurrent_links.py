...
def _GetDefaultConcurrentLinks(is_lto):
  ...
  if sys.platform in ('win32', 'cygwin'):
    ...
    # on a 64 GB machine.
    mem_limit = max(1, stat.ullTotalPhys / (2 * (2 ** 30)))  # total / 2GB
    ...
    try:
      ...
      # 2GB per ld process allows for some more bloat.
      return max(1, avail_bytes / (2 * (2 ** 30)))  # total / 2GB
    except Exception:
      return 1
  else:
